    ...
    with torch.no_grad():
        for iteration, batch in enumerate(validation_data_loader, 1):
            ...
            for mul in mul_list:
                ...
                I2_N = torch.cat((I2_list[0][:, :, :H_Half, :W_Half], I2_list[1][:, :, :H_Half, W_margin:]),
                                 dim=3)  # NW+NE
                I2_S = torch.cat((I2_list[2][:, :, H_margin:, :W_Half], I2_list[3][:, :, H_margin:, W_margin:]),
                                 dim=3)  # SW+SE
...
if __name__ == '__main__':
    ...
    parser.add_argument('--Dataset', type=str, required=True,
                        choices=['ucf101', 'vimeo', 'SNU-FILM-all', 'Xiph_HD', 'X4K1000FPS'])
